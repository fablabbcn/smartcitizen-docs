...
### Modify sensor specific parameters - `control`

Each sensor has potentially parameters than can be customized, which are accessed by the `control` command. For any sensor, you can do:

```
control temperature
```

And it will show you available commands. As an example, we show the temperature and relative humidity offset below.

#### Modify temperature and relative humidity offset - `control`
...
Finally, external sensors, connected via the [auxiliary port](/hardware/kit/features/#auxiliary-connector/) can also be modified.
...
!!! info "Default value"
    The default value for external sensors is 0degC and 0 %RH.
