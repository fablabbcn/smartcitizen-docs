...
Most CO2 sensors in the _low cost range_ are [NDIR (NonDispersive InfraRed sensors)](https://en.wikipedia.org/wiki/Nondispersive_infrared_sensor), a type of _sensor_ that emits light and uses _infrared absorption_ to measure CO2. This is the principle that the [SCD30](/knowledge/air/co2/Sensirion_SCD30/) uses, which you can directly plug into the [Auxiliary port](/hardware/Auxiliary Connector/) of the Smart Citizen Kit (2.1 and above). There are some other types of sensor, for instance the [SCD4X](/knowledge/air/co2/Sensirion_SCD4X/) which is on its way to be supported on the SCK.
