...
The {{ short_name}} is designed to be **flexible**, and many custom configurations are possible by adding various [third-party sensors](/knowledge/). This can be the starting point for more complex setups, monitoring [air](/knowledge/air/), [soil, or water](/knowledge/soil-water/) parameters. Any _custom_ configuration based on the {{ short_name }} also connects to the Smart Citizen Platform with all features freely available. Some more complex configurations are what we call the [Smart Citizen Stations](/hardware/stations/).
...
!!! info "A note about versions and compatibility"
    All the {{ short_name }}2.X generations above 2.1 (included) are fully compatible regarding firmware, and their respective [Urban Boards](/hardware/boards/urban-board/) are fully compatible with any [Data Board](/hardware/boards/data-board/) of the _2.X series_ (above and including 2.1).
...
!!!info "Internal memory"
    An additional data collection feature, used in both **online** and **offline** options, is the [internal memory](/hardware/kit/features/#internal-memory). This feature saves data regardless of the mode configured, using limited internal memory to ensure no data gets lost. The Kit will automatically publish or store new data once the primary data storage solution is functional again.
